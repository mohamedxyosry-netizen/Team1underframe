...
function getCurrentUser() {
  try {
    ...
    /*
      حل مشكلة المستخدم متعدد الأقسام:

      لو بيانات sessionStorage القديمة لا تحتوي على
      secondRole أو extraRoles، نرجعها من users.
    */
    ...
    if (userData) {
      ...
      if (!user.secondRole && userData.secondRole) {
        user.secondRole = userData.secondRole;
      }

      if (!Array.isArray(user.extraRoles) && userData.extraRoles) {
        user.extraRoles = userData.extraRoles;
      }
    }
...
function login() {
  ...
  /*
    مهم:
    حفظ الدور الثاني مع بيانات المستخدم.
  */
...
function protectPage() {
  ...
  // ============================================================
  // السماح لكل المستخدمين المسجلين بالدخول إلى Priority Item
  // ============================================================
  if (currentPage === "priority.html") return;
...
function openSection(section) {
  ...
  if (
    user.role !== "admin" &&
    user.role !== section &&
    user.secondRole !== section &&
    !(Array.isArray(user.extraRoles) &&
      user.extraRoles.includes(section))
  ) {

    alert(
      "❌ ليس لديك صلاحية للدخول إلى القسم هذا"
    );
...
function getProblemStatus(record) {
  ...
  if (!record || record.done !== "لا") {
    return "none";
  }

  const meta =
    readProblemMeta(
      record.notCompletedReason
    );
...
function isProblemRecord(record) {
  ...
  return !!record &&
    record.done === "لا" &&
    getProblemStatus(record) !== "resolved";
}
...
function isProblemPendingReview(record) {
  ...
  return !!record &&
    record.done === "لا" &&
    getProblemStatus(record) === "pending_review";
}
...
function normalizeRecord(record) {
  ...
  const meta =
    readProblemMeta(
      normalized.notCompletedReason
    );

  normalized.problemStatus =
    meta?.status ||
    (
      normalized.done === "لا"
        ? "open"
  ...
  normalized.problemResolvedBy =
    meta?.resolvedBy || "";

  normalized.problemResolvedAt =
    meta?.resolvedAt || "";

  normalized.problemReviewedBy =
    meta?.reviewedBy || "";

  normalized.problemReviewedAt =
    meta?.reviewedAt || "";
...
async function getNotificationsData() {
  ...
  return notifications
    .map(notification => {
      ...
      const record =
        recordsMap.get(
          String(notification.problem_id)
        );
...
async function markProblemNotificationRead(
  problemId
) {
  ...
  try {
    ...
    if (result.error) {
      ...
      console.error(
        "Mark Problem Notification Read Error:",
        result.error
      );
...
function getVisibleProblemRecords(
  records,
  user = getCurrentUser()
) {
  ...
  /*
    السماح للمستخدم برؤية كل الأقسام
    التي لديه صلاحية عليها.
  ...
  */
  ...
  if (Array.isArray(user.extraRoles)) {
...
async function getProblemRecordsForCurrentUser() {
  ...
  return getVisibleProblemRecords(
    records,
    user
  );
}
...
async function getProblemsCountForCurrentUser() {
  ...
  return (
    await getProblemRecordsForCurrentUser()
  ).length;
}
...
async function getPendingReviewProblems() {
  ...
  return records.filter(
    isProblemPendingReview
  );
}

async function updateProblemMeta(
  recordId,
  meta
) {
...
async function resolveProblem(
  ...
  if (!cleanReason) {
    ...
    alert(
      "من فضلك اكتب سبب/تفاصيل إتمام حل المشكلة"
    );
  ...
  if (
    !record ||
    !isProblemRecord(record)
  ) {

    alert(
      "❌ المشكلة غير موجودة أو تم التعامل معها بالفعل"
    );
  ...
  /*
    السماح للمستخدم بحل المشكلة
    إذا كانت المشكلة في أي قسم من أقسامه.
  ...
  */
  ...
  if (user.role !== "admin") {
    ...
    if (
      ...
      alert(
        "❌ ليس لديك صلاحية لتعديل هذه المشكلة"
      );
  ...
  const notificationResult =
    await window.supabaseClient
      ...
      .insert({
        ...
        message:
          `🛠️ ${user.name} قام بحل مشكلة ` +
          `العربية: ${record.carNumber || "-"} ` +
  ...
  if (notificationResult.error) {
    ...
    console.error(
      "Problem Notification INSERT Error:",
      notificationResult.error
    );

    alert(
      "⚠️ تم تسجيل حل المشكلة، ولكن حدث خطأ أثناء إرسال الإشعار للمدير\n\n" +
  ...
  alert(
    "✅ تم تسجيل حل المشكلة وهي الآن في انتظار مراجعة المدير"
  );

  return true;
}

async function approveResolvedProblem(
  recordId
) {
  ...
  if (
    ...
    alert(
      "❌ اعتماد حل المشكلة متاح للمدير فقط"
    );
  ...
  if (
    !record ||
    !isProblemPendingReview(record)
  ) {

    alert(
      "❌ هذه المشكلة ليست في انتظار مراجعة المدير"
    );
  ...
  await markProblemNotificationRead(
    recordId
  );
  ...
  alert(
    "✅ تم اعتماد حل المشكلة"
  );

  return true;
}

async function rejectResolvedProblem(
  recordId,
  reason
) {
  ...
  if (
    ...
    alert(
      "❌ رفض حل المشكلة متاح للمدير فقط"
    );
  ...
  if (
    ...
    alert(
      "❌ المشكلة ليست في انتظار المراجعة"
    );
  ...
  if (ok) {
    ...
    alert(
      "↩️ تم إرجاع المشكلة للعامل لمتابعتها مرة أخرى"
    );
  }
...
function askResolveProblem(
  recordId
) {

  const reason =
    prompt(
      "اكتب تفاصيل حل المشكلة أو ما تم عمله:"
    );
...
function askRejectResolvedProblem(
  recordId
) {
...
function renderProblemDetails(
  ...
  const status =
    getProblemStatus(record);

  const reason =
    getProblemReason(record);
  ...
  if (status === "open") {
    ...
    actions = `
      ...
      <button
        ...
        onclick="askResolveProblem('${escapeHTML(record.recordId)}')"
      >
        ✅ تم حل المشكلة
      </button>
    ...
    actions = `
      ...
      <div class="info-box">
        ...
        <small>
          تم تسجيل الحل بواسطة:
          ${escapeHTML(
            record.problemResolvedBy || "-"
          )}
        </small>
    ...
    if (
      ...
      actions += `
        ...
        >
          ...
          <button
            class="pending-button"
            onclick="approveResolvedProblem('${escapeHTML(record.recordId)}')"
          >
            ✅ اعتماد الحل
          </button>
          ...
          <button
            class="pending-button"
            onclick="askRejectResolvedProblem('${escapeHTML(record.recordId)}')"
          >
...
function renderProblemDepartment(
  department,
  records
) {
    ...
    return `
      ...
      <section class="work-box">
        ...
        <div class="no-records-box">
          لا توجد مشاكل في هذا القسم لهذه العربية
        </div>
      ...
      <div class="pending-list">
        ...
        ${devices.map(
          (device, index) => {
            ...
            const id =
              `problem_${department}_${index}_${Date.now()}`;
            ...
            return `
              ...
              <div class="pending-item">
                ...
                <button
                  class="pending-button"
                  onclick="toggleProblemDevice('${id}')"
                >
...
async function saveInstallation() {
  ...
  const reason =
    document.getElementById(
      "installationNotCompletedReason"
    )?.value.trim() ||
...
async function updateDashboardStats() {
  ...
  if (elements.problems) {
    elements.problems.innerText =
      visibleProblems;
  }

  if (elements.review) {
    elements.review.innerText =
      reviewCount;
  }

  const reviewMenu =
    document.getElementById(
      "resolvedProblemsMenu"
    );

  if (reviewMenu) {

    reviewMenu.style.display =
      user.role === "admin"
        ? ""
        : "none";
  }
    ...
    requests.map(
      request => `
        ...
        <div class="registered-box">
          ...
          <button
            class="pending-button"
            onclick="approveDeleteRequest(
              '${escapeHTML(request.id)}',
              '${escapeHTML(request.record_id)}'
            )"
          >
...
async function approveDeleteRequest(
  ...
  if (
    ...
    alert(
      "❌ الموافقة على الحذف متاحة للمدير فقط"
    );
  ...
  try {
    ...
    alert(
      "✅ تم حذف السجل والموافقة على الطلب"
    );
...
function calculateCarNumber(
  ...
  return (
    `${baseNumber + positions[name]}-${name}-${number}`
  );
}
