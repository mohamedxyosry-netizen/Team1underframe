...
function login() {
    ...
    const foundUsername =
        Object.keys(users).find(function(name) {
            return (
                name.toLowerCase() ===
...
function openSection(section) {
    ...
    if (section === "search") {
        alert("ليس لديك صلاحية للدخول إلى سجل البيانات ❌");
        return;
    }
...
function setupDashboard() {
    ...
    const deleteRequestsMenu =
        document.getElementById("deleteRequestsMenu");
...
async function getDepartmentRecordsFromSupabase(storageName) {
    ...
    const department =
        getDepartmentFromStorage(storageName);
...
const DELETE_REQUESTS_STORAGE = "deleteRequests";
...
async function requestDeleteRecord(storageName, index) {
    ...
    const records =
        await getDepartmentRecordsFromSupabase(storageName);
    ...
    let allowed =
        ownDepartments[user.role] === storageName;
    ...
    const pendingResult =
        await window.supabaseClient
            .from("delete_requests")
            .select("id")
            .eq("record_id", String(record.recordId))
            .eq("status", "pending");

    if (pendingResult.error) {

        console.error(
            "DELETE REQUEST CHECK ERROR:",
            pendingResult.error
        );

        alert(
            "❌ حصل خطأ أثناء التحقق من طلب الحذف\n\n" +
    ...
    const reason =
        prompt("اكتب سبب طلب حذف هذا البيان:");
    ...
    const deleteRequest = {
        ...
        section: getSectionName(storageName),
    ...
    if (
        typeof displayDepartmentRecords === "function"
    ) {
    ...
    if (
        typeof updateDeleteRequestsCount === "function"
    ) {
...
async function deleteRecordDirectly(storageName, index) {
...
async function deleteRecordDirectlyById(recordId) {
    ...
    try {
        ...
        if (
            typeof displayAllRecords === "function"
        ) {
...
function getSectionName(storageName) {

    const names = {

        wiringRecords: "قسم Wiring",
        ltRecords: "قسم LT",
        htRecords: "قسم HT",
        installationRecords: "قسم التركيبات",
        finalRecords: "قسم الفنش النهائي"

    };

    return names[storageName] || storageName;
}
        ...
        container.innerHTML += `
            <div class="record">
                ...
                ${
                    ...
                    ? `
                        <p>
                            <strong>تم التسجيل بواسطة:</strong>
                            ${escapeHTML(record.addedBy)}
                        </p>
                    `
                    : ""
                }
    ...
    try {
        ...
        const completed =
            records.filter(function(record) {
                return record.done === "نعم";
            }).length;

        const pending =
            records.filter(function(record) {
                return record.done === "لا";
            }).length;
        ...
        if (elements.total) {
            elements.total.innerText = records.length;
        }

        if (elements.completed) {
            elements.completed.innerText = completed;
        }

        if (elements.pending) {
            elements.pending.innerText = pending;
        }

        if (elements.delayed) {
            elements.delayed.innerText = 0;
        }

        if (elements.today) {
            elements.today.innerText = todayCount;
        }
...
async function displayDeleteRequests() {
    ...
    try {
        ...
        if (result.error) {
            ...
            console.error(
                "❌ LOAD DELETE REQUESTS ERROR:",
                result.error
            );

            container.innerHTML = `
                <div class="error">
                    ❌ فشل تحميل طلبات الحذف
                    <br><br>
                    ${escapeHTML(
                        result.error.message
                    )}
            ...
            if (status === "pending") {
                ...
                buttons = `
                    ...
                    ">
                        ...
                        <button
                            type="button"
                            onclick="approveDeleteRequest('${requestId}')"
                            style="
                                flex:1;
                                min-width:140px;
                                padding:12px;
                        ...
                        <button
                            type="button"
                            onclick="rejectDeleteRequest('${requestId}')"
                            style="
                                flex:1;
                                min-width:140px;
                                padding:12px;
        ...
        console.error(
            "❌ DELETE REQUESTS ERROR:",
            error
        );

        container.innerHTML = `
            <div class="error">
                ❌ حصل خطأ أثناء تحميل طلبات الحذف
                <br><br>
                ${escapeHTML(
                    error.message
                )}
...
async function approveDeleteRequest(requestId) {
    ...
    if (requestResult.error) {
        ...
        console.error(
            "❌ GET DELETE REQUEST ERROR:",
            requestResult.error
        );

        alert(
            "❌ تعذر العثور على طلب الحذف\n\n" +
    ...
    if (!request) {
        alert("طلب الحذف غير موجود ❌");
        return;
    }

    if (request.status !== "pending") {
        alert("تم التعامل مع هذا الطلب من قبل.");
        return;
    }
    ...
    if (updateResult.error) {
        ...
        console.error(
            "❌ UPDATE DELETE REQUEST ERROR:",
            updateResult.error
        );

        alert(
            "⚠️ تم حذف البيان، ولكن حدث خطأ أثناء تحديث حالة الطلب:\n\n" +
    ...
    if (typeof displayAllRecords === "function") {
        await displayAllRecords();
    }
    ...
    if (typeof updateDeleteRequestsCount === "function") {
        await updateDeleteRequestsCount();
    }
}
...
async function rejectDeleteRequest(requestId) {
    ...
    if (!user || user.role !== "admin") {
        alert("هذه العملية متاحة للمدير فقط ❌");
        return;
    }
